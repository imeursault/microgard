...
class Layer:
    ...
    def __call__(self, x):
        return [neuron(x) for neuron in self.neurons] if len(self.neurons) > 1 else self.neurons[0](x)
...
class MLP:
    def __init__(self, num_inputs, num_outputs, hidden_size, num_hidden):
        self.hidden = [Layer(num_inputs if i == 0 else hidden_size, hidden_size) for i in range(num_hidden)]
